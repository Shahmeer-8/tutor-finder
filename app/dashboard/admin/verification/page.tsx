...
function AdminVerificationPage() {
  ...
  const handleReject = async (tutorId: string) => {
    ...
    try {
      ...
      alert(`Failed to reject tutor: ${err.message}`);
    } finally {
      setProcessingId(null);
    }
  };
      ...
      <main className="flex-1 p-6 md:p-8">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex justify-between items-center">
            <h1 className="text-2xl font-bold text-gray-900">Tutor Verification</h1>
          </div>
              ...
              {isLoading ? (
                ...
                <div className="divide-y divide-gray-100">
                  {pendingTutors.map((tutor) => (
                    <div key={tutor._id} className="p-6 hover:bg-gray-50/50 transition-colors">
                      <div className="flex flex-col md:flex-row justify-between gap-6">
                        ...
                        <div className="flex-1 space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-4 p-4 bg-gray-50 rounded-lg">
                            <div>
                              <span className="font-semibold text-gray-700 block mb-1">Education</span>
                              <span className="text-gray-600">{tutor.education || 'Not provided'}</span>
                            </div>
                            <div>
                              <span className="font-semibold text-gray-700 block mb-1">Subjects</span>
                              <span className="text-gray-600">{tutor.subjects?.join(', ') || 'None'}</span>
                            </div>
                          </div>

                          {tutor.bio && (
                            <div>
                              <span className="font-semibold text-sm text-gray-700 block mb-1">Bio</span>
                              <p className="text-sm text-gray-600 whitespace-pre-wrap">{tutor.bio}</p>
                            </div>
                          )}
                        </div>

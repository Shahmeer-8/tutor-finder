...
function AdminUsersPage() {
  ...
  const handleToggleBlock = async (user: User) => {
    ...
    if (
      !window.confirm(`Are you sure you want to ${actionText} ${user.name}?`)
    ) {
    ...
    try {
      const { user: updatedUser } = await adminService.toggleBlockStatus(
        user.id,
        newStatus,
      );
      setUsers(
      ...
      showToast(`User successfully ${actionText}ed`, "success");
    } catch (err: any) {
      showToast(`Failed to ${actionText} user: ${err.message}`, "error");
    } finally {
      setProcessingId(null);
    }
  };
            ...
            <CardContent className="p-0">
              ...
              {isLoading ? (
                ...
                <EmptyState
                  ...
                  description={`No users found for the filter: ${activeTab}`}
                  className="m-6"
                />
              ) : (
                      ...
                      {users.map((u) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 capitalize">
                            <Badge
                              variant={u.role === "tutor" ? "info" : "default"}
                            >
                              {u.role}
                            </Badge>
                          </td>
